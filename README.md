...
<оп> ::=  <буквенный символ операции>

<нс> ::=  <ЗНАК НОВОЙ СТРОКИ>
...
<образец> ::= <оп> | <метаимя> | <образец> <образец> | 
`(` <образец> `)`
...
<правило расширения> ::= <метаимя> `::+` <образец> `==>` <заготовка> <нс>
...
`<блок>` `::+` <оп> <образец> <оп> <оп> ... `==>` <заготовка> <нс>

или

`<блок>` `::+` <оп> <образец> <итд> `==>` <заготовка> <нс>

или

<итд> `::+` <оп> <образец> <итд> `==>` <заготовка> <нс>

или

<итд> `::+` <оп> <оп> ... `==>` <заготовка> <нс>
...
<итд 2> ::+  ELSE <команда> END CASE ==> \
             (<команда>)

<итд 2> ::+  END CASE ==> \
             ()

<блок> ::+ WHILE <формула> DO <команда> END WHILE ==> \
           (loop (): \
            <формула> -> ((<команда>) ; loop()); \
            ()) 

<блок> ::+ FOR <имя> <итд 3> <команда> END FOR ==> \
           (start = 0; end = INFTY; step = 1; (<итд 3>); \
            result() = (); \
            loop(<имя> = start): \
            <имя> >= end -> result(); \
            (<команда>); \
