...
список формул разделенных запятыми заключенный в круглые скобки `(<ф1>, ...)`. 
...
Данная команда имеет вид `<о> = <ф> , <ок>`, где <о> — образец, содержащий одно определяемое имя, 
<ф> — формула, задающая начальное значение, а <ок> — команда определения имён с
...
`<им> ( <оп> )`, где <им> это имя метки, а <оп> это возможно пустая последовательность 
определений имён параметров вида `<и> = <ф>` разделённых запятой. 
...
выражения не приводит к вызову имени метки <им> с новыми параметрами, то
