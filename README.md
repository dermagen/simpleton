## Описание языка программирования ПРОСТЕЦ 1
...
правый.
   ...
   | Старшинство | Название категории операций | Характерные операции | Асоциация |
   |-------------|--------------------|----------------------|------------|
   | 1 | группа изменения знака | `-` `+` | (одноместные) |
   | 2 | группа умножения | `*` `/` | влево |
   | 3 | группа сложения | `+` `-` | влево |
   | 4 | группа минимум/максимум | `<>` `><` | влево |
   | 5 | отношения | `<` `>` `<=` `>=` | влево |
   | 6 | равенства | `==` `/=` | влево |
   | 7 | группа логической конъюнкции | `&` | влево |
   | 8 | группа логической дизъюнкции | `\|` | влево |
   | 9 | порождение новых последовательностей | `#` `##` | вправо |
   | 10 | группа функциональной абстракции | `=>` | вправо |
...
### 3.2.3. Команда определения локальных имён (трёхместная операция `=` `;`)
...
### 3.2.4. Команда совместного определения локальных имён (трёхместная операция `=` `,`)  
