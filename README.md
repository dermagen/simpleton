...
иметь грамматический смысл; при этом знак новой строки выступает в качестве
грамматического разделителя особого рода, отличающегося от разделителя-пробела или
разделителя-знака табуляции. Таковое отличие может быть нивелировано посредством
вставки перед концом строки знака обратной косой черты (`\`), сообщающего машинному
исполнителю кода что в данном месте конец строки должен быть проигнорирован и
следующая строка должна считаться продолжением текущей. Между знаком обратной косой черты
и знаком новой строки может находиться пробельный материал в виде последовательности
знаков пробела и знаков табуляции; вся цепочка знаков начинающаяся с `\` и заканчивающаяся
новой строкой считается «знаком склейки», логически эквивалентным одному пробелу
посреди регулярного кода и полностью изымаемым из последовательности входных
знаков посреди изображения строчного литерала (см. ниже).
...
комментарий начинается с восклицательного знака (`!`) и заканчивается знаком
непосредственно предшествующим знаку новой строки на той же строке. Сам знак новой строки 
...
> располагающийся между обратной косой чертой и новой строкой. Обратное вложение
...
<код знака> ::= <код '> | <код "> | <код ‹> | <код ›> | <код «> | <код »> | <код ~> | <код табуляции> | <код новой строки> | <универсальный код>
...
<имя элемента> ::= `зоп` | ... | `аоп` | ... | `формула` | `команда`

#### П 2.2. Грамматические элементы

Правила в этой части позволяют разделять элементы при помощи элемента
<внутристрочный пробельный материал> из основной грамматики. 

<образец> ::= <буквенный символ операции> | <метаимя> | <образец> <образец> | 
`(` <образец> `)`
...
<правило расширения> ::= <метаимя> `::+` <образец> `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

> NB. Поскольку правило грамматического расширения чувствительно ко всем символам
> включая пунктуацию, требуется чтобы всё правило записывалось на одной логической
> строке. Длинные правила можно разбить на несколько физических строк при помощи
> знака склейки строк (`\`), описанном выше в разделе 2.
...
4. Заготовка должна использовать каждое метоимя образца ровно один раз
...
иметь следующий вид (не больше одного правила на один символ):

#### П 2.3.1. Определение новых именованных констант

`<эф>` `::+` <буквенный символ операции> `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
`<зф>` `::+` <буквенный символ операции> `<зф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
~~~
> NB. локальные имена в заготовках при подстановке автоматически подменяются
> так, чтобы избежать конфликтов с именами в подставляемых выражениях


#### П 2.3.3. Определение новых видов формул группы умножения

`<уф>` `::+` `<уф>` <буквенный символ операции> `<зф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
`<сф>` `::+` `<сф>` <буквенный символ операции> `<уф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.5. Определение новых видов формул группы группы минимум-максимум

`<мф>` `::+` `<мф>` <буквенный символ операции> `<сф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.6. Определение новых видов формул группы отношения

`<оф>` `::+` `<оф>` <буквенный символ операции> `<мф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.7. Определение новых видов формул группы равенства

`<рф>` `::+` `<рф>` <буквенный символ операции> `<оф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.8. Определение новых видов формул группы логической конъюнкции

`<кф>` `::+` `<кф>` <буквенный символ операции> `<рф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
`<дф>` `::+` `<дф>` <буквенный символ операции> `<кф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
`<нф>` `::+` `<дф>` <буквенный символ операции> `<нф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.11. Определение новых видов формул группы абстракции

`<аф>` `::+` `<нф>` <буквенный символ операции> `<аф>` `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>

#### П 2.3.12. Определение новых именованных формул

`<эф>` `::+` <буквенный символ операции> <образец> <буквенный символ операции> `==>` <заготовка> <ЗНАК НОВОЙ СТРОКИ>
...
может содержать нумерованные или не нумерованные метаимена соответствующие категориям <формула>
и <команда>. Все эти метаимена должны быть упомянуты в заготовке ровно один раз.

Примеры:
~~~ ПРОСТЕЦ

 <эф> ::+ BEGIN <команда> END ==> \
          (<команда>) 

 <эф> ::+ IF <формула> THEN <команда 1> ELSE <команда 2> FI ==> \
          (<формула> -> (<команда 1>); <команда 2>) 

 <эф> ::+ WHILE <формула> DO <команда> END ==> \
          (loop (): <формула> -> (<команда> ; loop()); ()) 
